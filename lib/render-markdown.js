export function renderMarkdown(text) {
    ...
    let html = text
        // Headers
        .replace(/^### (.*$)/gim, '<h3 class="text-xl font-bold font-heading mt-6 mb-3 text-foreground">$1</h3>')
        .replace(/^## (.*$)/gim, '<h2 class="text-2xl font-bold font-heading mt-8 mb-4 tracking-tight text-foreground">$1</h2>')
        .replace(/^# (.*$)/gim, '<h1 class="text-3xl font-bold font-heading mt-10 mb-6 tracking-tight text-foreground">$1</h1>')
