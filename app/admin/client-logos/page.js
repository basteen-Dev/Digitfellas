...
export default function ClientLogosAdmin() {
    ...
    const [formData, setFormData] = useState({
        name: '', logo_url: '', website_url: '', description: '', sort_order: 0, is_published: true, logo_image: []
    })
    ...
    const handleCreate = () => {
        setEditing(null)
        setFormData({ name: '', logo_url: '', website_url: '', description: '', sort_order: 0, is_published: true, logo_image: [] })
        setDialogOpen(true)
    }
    ...
    const handleEdit = (logo) => {
        setEditing(logo)
        setFormData({
            name: logo.name || '', logo_url: logo.logo_url || '', website_url: logo.website_url || '', description: logo.description || '',
    ...
    const columns = [
        ...
        { key: 'description', label: 'Description', render: (row) => <div className="max-w-[200px] truncate" title={row.description}>{row.description || '-'}</div> },
    ...
    return (
        <div className="p-8">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">Brands & Clients</h1>
                <Button onClick={handleCreate}><Plus className="w-4 h-4 mr-2" /> Add Brand</Button>
            </div>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editing ? 'Edit' : 'Add'} Brand</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div><Label>Description</Label><Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Brief description..." /></div>
